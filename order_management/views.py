...
class VerifyRazorpayPayment(View):
    ...
    def _handle_payment_failure(self, data, order_id):
        ...
        payment_failed_status, _ = PaymentStatus.objects.get_or_create(status='payment_failed')
...
def handle_payment_failure(request,order_id):
    ...
    # Redirect to a retry payment page or show an error page
...
def complete_payment(request, order_id):
    """
    Complete the payment for an already created order and update payment status.
    """
    try:
        ...
        if request.method == "POST":
            payment_method = request.POST.get("payment_method")
                # Update or create payment details for the order
            if not order.payment_details:
                # If payment details do not exist, create new PaymentDetails
                ...
                payment_status = PaymentStatus.objects.get_or_create(status="Completed")[0]
                order.payment_details.payment_method = payment_method
            ...
            return redirect(reverse('razorpay_order_success', args=[order.id]))

        # Render the payment completion page if GET request
        return render(request, 'checkout/complete_payment.html', {'order': order})

    except Exception as e:
        messages.error(request, f"An error occurred: {str(e)}")
        return redirect(reverse('handle_payment_failure', args=[order_id]))
